...
export class PlannerElement extends connect(store)(LitElement) {
  ...
  private changeDuration(e: MouseEvent): void {
    ...
    store.dispatch(plannerSlice.setSpeed(this.distance / ((1000 * Math.max(15, duration)) / 60)));
  }
  ...
  private async handleScoringResult(result: ScoringResult) {
    ...
    if (this.currentTrack && this.currentTrack.timeSec.length > 1) {
      ...
      store.dispatch(plannerSlice.setSpeed((result.lengthKm / durationS) * 3600));
      this.duration = durationS;
    }
    store.dispatch(plannerSlice.setDistance(result.lengthKm * 1000));
